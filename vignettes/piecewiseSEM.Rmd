...
In version 2.0, we implement two solutions for obtaining the error variance to produce standardized coefficients for GLM, focusing for the moment on binary response models: the latent theoretic and the observation error approach.

In the latent theoretic approach, we assume a fixed error variance for the binomial distribution based on the link function: for logit, it is $\pi$^2/3 and for the probit, it is 1. This value is added to the predicted fits on the linear (link) scale and then the square-root of the variance of these values is taken to obtain the standard deviation.

In the observation error approach, a rough estimate of the error variance is obtained through the calculation of a pseudo-R^2, which is simply the correlation between the observed and predicted fits (in this case, in the original units). The error variance is again added to the variance of the predicted fits and the square-root is taken to obtain the standard deviation.
...
```{r}
# Create fake binomial data
binom.data <- data.frame(y = rbinom(100, 1, 0.5))

binom.data$x <- binom.data$y + runif(100, 0, 5)

# Fit using GLM
glm.mod <- glm(y ~ x, "binomial", binom.data)

# Extract linear beta
Beta <- summary(glm.mod)$coefficients[2, 1]
```

And apply the latent theoretic approach:
```{r}
# Extract predicted values on the link scale
preds <- predict(glm.mod, type = "link")

# Compute sd of error variance using theoretical variances
sd.y.LT <- sqrt(var(preds) + pi^2/3)

# Compute sd of x
sd.x <- sd(binom.data$x)

# Get standardized coefficient
Beta * sd.x / sd.y.LT

# Compare to piecewiseSEM output
coefs(glm.mod, standardize.type = "latent")

```
You will note that the `coefs` function has an argument `standardize.type` that currently accepts either the `'latent'` or `'observation'` values.

This approach also works for probit links:
```{r}
glm.mod2 <- update(glm.mod, family = binomial(link = "probit"))

Beta2 <- summary(glm.mod2)$coefficients[2, 1]

preds2 <- predict(glm.mod2, type = "link")

sd.y.LT2 <- sqrt(var(preds2) + 1)

Beta2 * sd.x / sd.y.LT2

coefs(glm.mod2, standardize.type = "latent")
```

To conduct the observation error approach:
```{r}
# Compute R based on correlation between predicted and observed
R <- cor(binom.data$y, predict(glm.mod, type = "response"))

sd.y.OE <- sqrt(var(preds)) / R

# Get standardized coefficient
Beta * sd.x / sd.y.OE

# Compare to piecewiseSEM output
coefs(glm.mod, standardize.type = "observation")
```
At the moment, standardized coefficients for GLMs are only obtainable for binary responses, and for fixed-effect only models. In the future, we hope to extend this technique to additional responses and hierarchical models.
