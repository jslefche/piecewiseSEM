...
![GLM sem](https://github.com/jslefche/jslefche.github.io/blob/master/img/GLM_sem.jpg?raw=true)
...
```{r}
summary(glmsem, direction = c("y1 <- y2"))
```
In option 2, the SEM can be updated to remove that test.
```{r}
summary(update(glmsem, y1 %~~% y2))
```
...
```{r}
summary(glmsem, conserve = T)
```

The user should be vigilant for these kinds of situations and ensure that both the specified paths AND the independence claims all make biological sense.
