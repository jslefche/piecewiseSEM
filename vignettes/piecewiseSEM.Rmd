...
Performing this exercise manually generates the same standardized coefficients as the function `coefs`:
...
This is problematic because the d-sep tests are dependent on the significance value. If the P-value is biased based on the direction of the test, then the goodness-of-fit of the model can be over- or underestimated.
...
*the standardized estimate for the logistic regression is different, based on new calculations (see `?coefs`)
